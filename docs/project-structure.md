...
| `packages/mcp` | **`@relavium/mcp`** — the inbound MCP client: the SDK fence and the network transports behind the SSRF pre-connect floor (the JSON-Schema→Zod compiler moved to `@relavium/shared`, [ADR-0092](decisions/0092-output-schema-is-validated-by-the-compiler-we-already-own.md) §1) ([ADR-0052](decisions/0052-inbound-mcp-client-package-lifecycle-registration.md), [ADR-0053](decisions/0053-mcp-network-transport-egress-security.md)). |
