...
| Database (local, Phase 1) | **SQLite + Drizzle ORM** | Tauri plugin available; the **desktop** store is encrypted at rest with SQLCipher. Node-side consumers (CLI, `@relavium/db` tests) use the **`better-sqlite3`** driver ([ADR-0021](decisions/0021-node-sqlite-driver-better-sqlite3.md)); the **CLI** store is **unencrypted at rest** — guarded by `0600`/`0700` owner-only permissions + the OS keychain, no credentials stored ([ADR-0050](decisions/0050-cli-history-db-at-rest-posture.md)). |
