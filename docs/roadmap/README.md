...
flowchart LR
    subgraph P1["Product Phase 1 — local-first BYOK (no account, no cloud)"]
        ...
        P25["Phase 2.5<br/>CLI consolidation<br/>(Home + modes + root-cause)"]
        P255["Phase 2.5.5<br/>hardening & remediation<br/>(parallel with 2.6, no gate)"]
        P26["Phase 2.6<br/>conversational authoring<br/>+ first-class CLI"]
        P3["Phase 3<br/>desktop<br/>(Tauri agent center)"]
        P4["Phase 4<br/>VS Code<br/>(standalone ext)"]
        P0 --> P1E --> P2 --> P25 --> P26 --> P3 --> P4
        P25 -. "concurrent, non-blocking<br/>(remediates pre-2.6 code)" .-> P255
    end
...
| **2.5.5** | Close out the 373-finding 2026-07-19 full-project review's verified defects against the Phase 0–2.5 codebase and the already-merged 2.6 slices (2.6.C, 2.6.F) — engine/LLM/persistence hardening, MCP/secrets, the CLI operator-safety net, TUI terminal safety, docs accuracy, and CI/test-infrastructure. | Phase 2.5 (subject-matter lineage only — runs **concurrently with the active Phase 2.6** and blocks none of it) | All nine sub-streams (2.5.5.A–I) land or are explicitly re-deferred; the three CRITICAL findings — the unredacted tool-approval preview, the non-atomic chat-turn persister write, and unsanitized `relavium run` terminal output — are closed; no 2.6 workstream is blocked or re-sequenced. | [phases/phase-2.5.5-hardening-and-remediation.md](phases/phase-2.5.5-hardening-and-remediation.md) |
...
| **M3 ✅** | **CLI + engine regression harness** *(achieved 2026-06-23, PR #43)* — the `relavium` CLI drives the engine end-to-end with a live `ink` TUI and a deterministic `--json` CI mode; a small fixture suite runs in CI on every engine change as the agreed regression gate for Phases 2.5–6. (Phase 2 shipped complete as **v0.1.1**, PR #58; the CLI consolidation/authoring work continues in Phases 2.5–2.6 before M4.) | 1 | Phase 2 · 2.D (run wiring), 2.F (`--json` / CI mode), 2.K (regression harness) |
| **M2.5.5** | **Hardening & remediation review closed** — the 373-finding 2026-07-19 full-project review's verified defects against the Phase 0–2.5 codebase (and the merged 2.6.C/2.6.F slices) are resolved or explicitly re-deferred; the three CRITICAL findings (an unredacted tool-approval preview, a non-atomic chat-turn persister write, unsanitized `relavium run` terminal output) are closed. Runs **concurrently with Phase 2.6** — not a gate on M4/Phase 3, and not itself gated by any 2.6 workstream. | 1 | Phase 2.5.5 · 2.5.5.A–2.5.5.I |
...
> ([ADR-0012](../decisions/0012-managed-inference-dual-mode.md)), managed inference
> (M6) ships first and decoupled from cloud execution (M7). Phase 2.5.5 (hardening &
> remediation) sits off this path entirely — it runs concurrently with Phase 2.6,
> remediating code that already shipped, and carries no blocking edge into or out of
> Phase 2.6 in the dependency graph above.

```mermaid
flowchart LR
    M0(("M0<br/>foundations")) --> M1(("M1<br/>LLM seam"))
    M1 --> M2(("M2<br/>engine e2e"))
