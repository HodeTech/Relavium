...
- **Product Phase 2 (cloud)** — an optional cloud execution layer and web portal
  added *alongside*, never replacing, the local-first surfaces. This is **build
  phase 5**, and every part of it is marked explicitly as Product Phase 2. It is
  hard-gated on all of Product Phase 1 being shipped and battle-tested by real
  users ([ADR-0008](../decisions/0008-local-first-phase-1-cloud-phase-2.md)).
...
**`packages/shared` → `@relavium/llm` → `@relavium/core` → CLI → desktop → vscode → cloud**
...
Phase 1, then the hard gate into Product Phase 2.

```mermaid
flowchart LR
    subgraph P1["Product Phase 1 — local-first (no account, no cloud)"]
        direction LR
    ...
    subgraph P2C["Product Phase 2 — cloud (optional, additive)"]
        P5["Phase 5<br/>cloud + portal"]
    end
...
| **5** | **Product Phase 2** — optional cloud execution (BullMQ/Redis/Postgres) + the web portal (usage/quota/license/enterprise), with a **transparent local→cloud switch** behind the engine's `ExecutionHost` interface, so the *same* `@relavium/core` runs in both modes with no fork. | All of Phase 1 (0–4), shipped + battle-tested ([ADR-0008](../decisions/0008-local-first-phase-1-cloud-phase-2.md)) | A team runs a workflow in the cloud, shares a run-replay URL, and approves a gate via an email link — same engine, no fork (mode flag via `ExecutionHost`); SSE delivers canonical events with lossless reconnection; multi-tenancy is provably isolated and the four-key-leak security audit passes; quota enforcement fires; transcripts are never synced; **all Phase 1 surfaces still run fully locally with no account, unchanged.** | [phases/phase-5-cloud.md](phases/phase-5-cloud.md) |
...
| **M3** | **CLI + engine regression harness** — the `relavium` CLI drives the engine end-to-end with a live `ink` TUI and a deterministic `--json` CI mode; a small fixture suite runs in CI on every engine change as the agreed regression gate for Phases 3–5. | 1 | Phase 2 · 2.D (run wiring), 2.F (`--json` / CI mode), 2.K (regression harness) |
...
| **M6** | **Product Phase 2 shipped** — cloud execution + the control-plane portal with a transparent local→cloud switch (one engine, no fork): accounts + multi-tenancy, lossless SSE, triggers + gate notifications, quota enforcement, enterprise (SSO/RBAC/audit); the multi-tenant security audit passes and Phase 1 local-first is regression-green. | **2** | Phase 5 · 5.G (auth + tenancy), 5.J (portal), 5.K (enterprise) |

## Critical path

> **The critical path is the engine: Phase 1 (`@relavium/llm` + `@relavium/core`).**
> Phase 0 only de-risks it; every surface (Phases 2–4) and the cloud layer (Phase 5)
> sit on top of it. A slip in Phase 1 slips everything. This is why the build order
> is engine-first and why the CLI (Phase 2) follows immediately as the first real
> consumer and regression harness — to prove the engine API ergonomics before any UI
> complexity, and to keep the engine green through every later phase.
...
flowchart LR
    ...
    M5 -. "hard gate (ADR-0008)" .-> M6(("M6<br/>cloud + portal"))
...
touches. They are the contract that lets six phases share one engine without drift.

1. **One engine, no fork.** `@relavium/core` + `@relavium/llm` are the only execution
   engine. Every surface (CLI, desktop, VS Code) and the cloud layer *consume* the
   same engine; cloud-only behavior lives behind the `ExecutionHost` seam (Phase 5),
   never in a parallel engine ([ADR-0008](../decisions/0008-local-first-phase-1-cloud-phase-2.md)).
...
3. **Zero platform-specific imports in the engine.** `@relavium/core` runs identically
   in Node, the Tauri WebView, the VS Code extension host, and (Phase 5) the Bun API
   — verified since Phase 1, relied on for bundling in Phase 4 and hosting in Phase 5.
...
5. **One Drizzle schema, two dialects.** A single Drizzle schema targets local SQLite
   (Phases 2–4) and cloud Postgres (Phase 5) with no fork; cost is always stored as
...
7. **Local-first, no account, through Phase 1.** Phases 0–4 require no cloud, no
   account, no server. Phase 5 is additive and must never break this — the final exit
   gate re-verifies that every Phase 1 surface still runs fully locally, unchanged
   ([product-constraints.md](../product-constraints.md)).
