...
- Any engine logic (`packages/core`), provider adapters (`packages/llm`), DB layer
  (`packages/db`), or UI (`packages/ui`) — Phase 1 and later.
...
- The per-provider conformance suite, fixtures, and the nightly live-API CI lane —
  ...
  created here.
- Over-scaffolding later packages: only `packages/shared` is built out now; other
  packages/apps are created as minimal placeholders (or not at all) until their phase.
...
fold in after 0.D and gate on 0.E. The critical-path items for this phase are
**0.C, 0.D, 0.E** (they feed M0 and unblock Phase 1).
...
- Create the directory skeleton from [../../project-structure.md](../../project-structure.md):
  ...
  Build out **only `packages/shared`** now; leave the rest as minimal placeholders
  (a `package.json` + `README.md` stub) or omit until their phase, per the
  no-over-scaffolding rule.
...
- **`RunEvent` union + `RunSchema`** per
  ...
  (`inputTokens`, `outputTokens`, `costUsd`, `cumulativeCostUsd`) and the
...
| 0.M3 — **M0: Foundations green** | CI green on push with remote cache; the seam lint fence and the standards are enforced; docs wired | 0.F, 0.G, 0.H |
...
| **Over-scaffolding.** Empty app/package shells with config that rots before they are built. | Low–Medium | Build out only `packages/shared` now (0.A); other packages stay minimal placeholders until their phase. |
