...
flowchart TD
    ...
    P265["Phase 2.6.5 — Core reliability<br/>48 CR items · 8 P0 blockers behind ADR-0078–0084<br/>W0+W1+W2 merged · W3 merged with a live blocker (28 of 48)<br/>W4 review-blocked on PR #87 — 5 blockers found and fixed"]
...
> **Live status — 28 of 48 closed. `W0`–`W2` merged clean; `W3` merged 2026-08-30 (PR #86) with one reproduced BLOCKER and nine verified findings open — see the `W3` residuals in [deferred-tasks.md](deferred-tasks.md). `W4` is REVIEW-BLOCKED on PR #87 behind [ADR-0088](../decisions/0088-the-mcp-boundary-is-hostile.md): a systematic review found five merge blockers (an orphaned MCP child on a signal, an optional validated `fetch`, a bypassable transport byte bound, discovery paging past its budget, and two cloud-metadata endpoints reachable through the local opt-in) — all reproduced, all fixed on the branch, awaiting re-review.**
>
