...
Under `--json`, each entry is one NDJSON record — `{ kind, slug, name, tags, path, valid, lastRun }`, where `lastRun` is `{ runId, status, completedAt }` or `null` for a never-run workflow (and is omitted entirely for agents). See [Read-command `--json` output](#read-command---json-output).
