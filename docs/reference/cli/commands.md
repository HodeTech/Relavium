...
- **`relavium models pricing <model> --provider <slug> [--input <usd> --output <usd>] [--cached <usd>] [--image <usd-per-image>] [--audio <usd-per-second>] [--video <usd-per-second>]`** hand-enters the price of a model. It MAY be a model the generated catalog (`packages/llm/src/catalog/`) already prices — **the USER outranks the catalog** ([ADR-0071](../../decisions/0071-models-dev-as-the-model-metadata-source.md) §5): a negotiated rate, an enterprise discount, or a price the snapshot has not caught up with wins over the shipped one (2.5.G S10, [ADR-0065](../../decisions/0065-provider-economics-and-extensibility.md) §1–2). Token prices are **USD per million tokens** (`--input` prompt, `--output` completion, `--cached` cache-read); **media prices are per BILLED UNIT** — `--image` per *image*, `--audio` and `--video` per *second* ([ADR-0044](../../decisions/0044-media-access-governance-read-media-save-to-cost.md) §3's canonical units; [ADR-0089](../../decisions/0089-media-correctness-four-boundaries.md) §4). All are stored as integer micro-cents (`usd × 1e8`, never a float), and the two denominators carry **separate implausibility ceilings** — $1000 per image is already absurd where $1000/Mtok is merely expensive. **`--input`/`--output` are given together or not at all**, and a **media-only** invocation is legitimate: it is how you satisfy a `strict_cost_cap` refusal on a model the catalog already prices for tokens, without writing a user row that would then outrank the catalog for tokens too. An omitted flag leaves its column alone (the store preserves it); a stated `0` is believed and recorded as *free*, which for the media columns is distinct from *unpriced*. The row is written as `source='user'` and a live `models refresh` **never** clobbers it. Overriding a catalog price is **not silent**: the command echoes the catalog price it replaces, so the divergence is stated at set-time (ADR-0071 §5). This also **closes the cost-cap gap** ([ADR-0064](../../decisions/0064-live-model-catalog.md) §6) for a model the catalog does NOT price: before, an unknown model had no price, so `budget.max_cost_microcents` / `[chat].max_cost_microcents` **degraded to allow** for it; once user-priced, the cap is enforced (pre-egress **and** realized) on `run`, a `run` resumed via [`relavium gate`](#relavium-gate), `chat` / `chat-resume` (incl. a `/clear` rebuild, re-read fresh), the Home chat, and one-shot `agent run`. `--clear` retires the override; the model falls back to the catalog price, or to unpriced if the catalog has none. Guards (each a clean exit `2`, nothing written): an **unregistered provider** is refused (register it first with `relavium provider add`); a **negative / non-finite / implausibly-large** price is refused (per-unit and per-Mtok have their own ceilings); an invocation with **no price at all**, or a **half-stated** `--input`/`--output` pair, is refused; `--clear` beside **any** price flag — media included — is refused rather than half-applied; a price under a provider the catalog does **not** anchor the model to is refused (it would never apply); and the **same model id already user-priced under a *different* provider** is refused (the overlay keys by model id, so a second provider's price could not be distinguished — use a distinct id or re-price under that provider).
...
- **`--json`** ([ADR-0049](../../decisions/0049-cli-machine-output-contract.md)) emits **one NDJSON record per line**, stdout-pure, key-free:
  ...
  - `relavium models pricing --json` — one record: `{ model, provider, source, inputCostPerMtokMicrocents, outputCostPerMtokMicrocents, cachedInputCostPerMtokMicrocents, mediaImageCostMicrocents, mediaAudioCostMicrocents, mediaVideoCostMicrocents, overriddenCatalogPrice }` (the stored integer micro-cents; `source` is always `user`). A field is **omitted** when that flag was not given — the columns were preserved, not written, and reporting a `0` would claim a price nobody set. `overriddenCatalogPrice` is `null` when the catalog does not price the model.
