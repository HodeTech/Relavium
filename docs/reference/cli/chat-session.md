...
A reseat is a **host-side** move — the engine is unchanged. It reconstructs the conversation and resumes a **new `AgentSession`** instance on the chosen model; the **session id and its `history.db` row continue unbroken**. Two things named "the transcript" behave differently, and the distinction matters:
...
When the chosen model supports reasoning, the picker offers the effort tiers **inline as a sub-step of the same pick** — the tier rides into the reseat and is bound on the new instance, so it is in force from the reseated session's **first** turn ([ADR-0066](../../decisions/0066-normalized-reasoning-effort-control.md)). (An effort change on the model you are *already* on is [`/effort`](#the-multi-turn-loop) — a per-turn override, no reseat.)

Each **assistant** message is persisted with the `model_catalog` id of the model that actually produced it — resolved from the turn's last billed egress, so it is failover-aware. User rows carry none. A turn that never billed, or a model absent from the catalog (a live-catalog or custom-`base_url` model), leaves it **null**.

> **Not yet shipped (2.6.C).** The per-model **cost breakdown** that surfaces this attribution — assigning null-`modelId` rows to an explicit **`unknown` (pre-attribution)** bucket rather than dropping them — is the open 2.6.C residual. Today [`/cost`](#the-multi-turn-loop) reports the session **total** only.
...
| `/models` | Open the live catalog **picker** ([ADR-0064](../../decisions/0064-live-model-catalog.md)) — **interactive-only**. In a **chat** an accepted pick **reseats** the live session onto that model (§ [Model reseat](#model-reseat-models), **2.5.G**), with the reasoning-effort sub-step riding into the same pick. In the **bare Home** it instead writes the global `[preferences].default_model` (+ the effort tier) for the *next* session ([ADR-0063](../../decisions/0063-cli-config-write-contract.md) §1). Opening the picker changes nothing — the effect lands only on an explicit selection. Under `--json` / a plain non-TTY there is no overlay: nothing is reseated and an actionable hint is printed. |
...
| `/scrollback` | Dump the transcript into the terminal's **native scrollback** to scroll, search, select and copy it — then press Enter to return ([ADR-0068](../../decisions/0068-full-screen-tui-renderer-ink7-harness.md) §e). Suspends the full-screen renderer and restores every terminal mode on exit. One of the three copy-and-search **hatches** that give the alt buffer's transcript back to the tools you already have (the alt screen has no native scrollback of its own). Not available before the first turn. Chat-only. |
| `/edit` | Open the transcript **read-only** in `$EDITOR` (to search or copy) — edits are never read back ([ADR-0068](../../decisions/0068-full-screen-tui-renderer-ink7-harness.md) §e). Suspends the renderer like `/scrollback`. Not available before the first turn. Chat-only. |
| `/copy` | Copy the **whole unwrapped transcript** to the system clipboard over **OSC 52** ([ADR-0068](../../decisions/0068-full-screen-tui-renderer-ink7-harness.md) §e) — works over SSH and inside a container; suspends nothing. OSC 52 has no acknowledgement, so the copy is *emitted*, not confirmed ([accessibility.md](accessibility.md)). Not available before the first turn. Chat-only. |
