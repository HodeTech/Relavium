...
- the **rendered (on-screen)** transcript is **not** carried. The reseated view is seeded with the header facts (model, cumulative cost, turn count) and the switch notice only. On the **inline** renderer this is invisible — the prior lines were already printed and survive in the terminal's own scrollback. On the **full-screen alt screen**, which is the TTY default ([ADR-0068](../../decisions/0068-full-screen-tui-renderer-ink7-harness.md) §e), there is no native scrollback to fall back on, so the conversation **disappears from the screen**. The durable `history.db` transcript and the new model's context are intact — this is a *view* gap, not data loss.

> **Open (2.6.C).** Carrying the rendered transcript across a reseat on the full-screen renderer — so the conversation stays visible with the switch notice as an inline marker beneath it — is the tracked 2.6.C fix.
...
> **Not yet shipped (2.6.C).** The per-model **cost breakdown** that surfaces this attribution — assigning null-`modelId` rows to an explicit **`unknown` (pre-attribution)** bucket rather than dropping them — is the open 2.6.C residual ([ADR-0059](../../decisions/0059-cli-mid-session-model-reseat.md), [ADR-0070](../../decisions/0070-durable-per-model-session-cost-attribution.md)). Today [`/cost`](#the-multi-turn-loop) reports the session **total** only.
