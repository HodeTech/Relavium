...
A session **binds one agent for its whole lifetime** — there is no mid-session *agent* switching ([agent-session-spec.md](../contracts/agent-session-spec.md#what-a-session-is-and-is-not)). The **model**, by contrast, *is* switchable mid-session since 2.5.G: **`/models`** performs a host-side **reseat** ([below](#model-reseat-models)), which starts a **new `AgentSession` instance** bound to the new model. Each instance still has exactly one model, so ADR-0024's one-model-per-lifetime rule is **refined, not reversed** ([ADR-0059](../../decisions/0059-cli-mid-session-model-reseat.md)).
...
`/models` opens the live model picker ([ADR-0064](../../decisions/0064-live-model-catalog.md)) and, **in a chat**, switches the session's model in place — the **reseat** ([ADR-0059](../../decisions/0059-cli-mid-session-model-reseat.md)). In the **bare Home** (no live session) the same command instead writes the durable `default_model` preference ([ADR-0063](../../decisions/0063-cli-config-write-contract.md)); that write-default path is spec'd in [home.md](home.md).

A reseat is a **host-side** move — the engine is unchanged. It reconstructs the transcript and starts a **new `AgentSession.resume`** bound to the chosen model, carrying the **cumulative cost and turn count** forward, so the session id and its `history.db` row continue unbroken. What it does **not** carry is the point of its notice:

- the new instance memoizes a **fresh fallback chain** for the chosen model, built exactly as a *new* session on that model would build one. The original agent's `fallback_chain` is **not** carried across a switch — that chain belonged to the old model.
- the transcript continues **text-only**: the new model sees the conversation's text, **not** prior tool calls or file contents. A one-line notice discloses this on every switch. (Full-fidelity tool-context is a Phase-3 persister + schema extension.)
- each message is persisted with the `modelId` that actually produced it (failover-aware), so a reseated session's spend is attributable per model. Rows written before that wiring — and any failover edge that leaves it unset — carry a **null** `modelId` and are reported in an explicit **`unknown` (pre-attribution)** bucket rather than dropped, so legacy history is never silently lost from the totals.

A reseat may be followed by a **reasoning-effort sub-step** when the chosen model supports it — the picker offers the effort tiers inline, and the choice applies from the next turn ([ADR-0066](../../decisions/0066-normalized-reasoning-effort-control.md); the same normalization `/effort` uses).
...
| `/models` | Open the live catalog **picker** ([ADR-0064](../../decisions/0064-live-model-catalog.md)). In a **chat** it **reseats** the live session onto the chosen model (§ [Model reseat](#model-reseat-models) — a host-side swap, ADR-0059, **2.5.G**), optionally followed by the reasoning-effort sub-step. In the **bare Home** the same picker instead writes the durable `default_model` for the *next* session ([ADR-0063](../../decisions/0063-cli-config-write-contract.md)); see [home.md](home.md). Opening the picker changes nothing — the effect lands only on an explicit selection. |
| `/effort [tier]` | Set the **reasoning-effort** tier — `off` / `low` / `medium` / `high` / `max` ([ADR-0066](../../decisions/0066-normalized-reasoning-effort-control.md)); bare `/effort` shows the current tier + the options. A **per-turn session override** on the live session — **no reseat** (unlike `/models`). Applies from the next turn; only offered for a model that supports reasoning. Chat-only. |
