...
| `input_schema` | no | JSON Schema describing the input this agent expects. **Purely additive metadata — the engine never reads it**, and it does not change run-time execution. It round-trips through export/import and is stored on the agent row; the type-safe node chaining and VS Code completion it is intended to drive are **not built yet** (no consumer exists in `apps/vscode-extension` or `packages/ui` today). |
| `output_schema` | no | JSON Schema describing the output this agent produces. **Unlike `input_schema`, this DOES affect a run**: it is lowered to the seam's `responseFormat` and the returned content is validated against it, a miss failing the node with `validation` (`retryable: false`) — see [agent-runner.md](../shared-core/agent-runner.md) and [ADR-0092](../../decisions/0092-output-schema-is-validated-by-the-compiler-we-already-own.md). |
...
- **Inline** — self-contained: `{ id, transport, … }`, where the transport (`stdio | http | websocket`, plus the deprecated `sse` alias of `http` for older servers) dictates the required connection field — `stdio` needs a `command` (with optional `args`/`env`); a network transport needs a `url` (and may set `allow_local_endpoint` to opt into a private/loopback endpoint). Any transport may raise its connect deadline with `connect_timeout_ms`. **A `websocket` server must be local**: a remote `wss://` one is rejected at parse-adjacent admission (exit `2`) because its transport can be neither address-pinned nor byte-bounded — declare it as `http` (Streamable HTTP) instead ([ADR-0088](../../decisions/0088-the-mcp-boundary-is-hostile.md) §2.3).
- **By-name `ref`** — `{ ref: <registration-name>, tools_allowlist? }`: identity AND connection come from a `[[mcp_servers]]` registration ([config-spec.md](config-spec.md)); the inline connection fields — `id`, `transport`, `command`, `args`, `env`, `url`, `allow_local_endpoint`, `connect_timeout_ms` — are forbidden alongside `ref` (the registration provides them). `tools_allowlist` is the only field that may accompany it.

**`connect_timeout_ms`** (optional, any transport) raises this server's connect deadline in milliseconds, up to **600 000**. A value above the ceiling, zero, a negative or a non-integer is **rejected at parse** (exit `2`), never clamped — running under a limit its author did not write is how a bound becomes a surprise instead of a contract ([ADR-0088](../../decisions/0088-the-mcp-boundary-is-hostile.md) §1.4, the admission shape [ADR-0086](../../decisions/0086-absolute-admission-ceilings-on-authored-values.md) established). Omitted, the per-transport default applies — see the deadline table in [../shared-core/mcp-integration.md](../shared-core/mcp-integration.md#connect-and-call-deadlines). The field exists because the canonical `command: npx` example can outrun any sane default on a cold cache.

A `stdio` entry's **`env` may not declare a name that redirects the interpreter, the dynamic loader, or a tool's configuration** — `NODE_OPTIONS`, `PATH`, `ZDOTDIR`, `BASH_ENV`, `HOME`, `LD_*`, `DYLD_*`, `GIT_*`, `PYTHON*`, `NPM_CONFIG_*`, `BASH_FUNC_*` and the rest of the shared denylist. This is an **authored error**: the declaration is rejected at parse (exit `2`) rather than silently dropped at spawn, because an author who wrote it believed it would take effect. The match is case-insensitive, and the same rule governs `run_command`'s `declared_env` and a `[[mcp_servers]]` registration — one list, in `@relavium/shared`, so the two process hosts cannot drift ([ADR-0084](../../decisions/0084-consent-before-a-local-mcp-spawn.md) §4). A server's **credentials** belong in `env` as `{{secrets.*}}` references, which are unaffected.
