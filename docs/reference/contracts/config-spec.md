...
mouse = false                      # ADR-0068 §e (2.6.F Step 5e): terminal mouse reporting (DECSET 1000+1006) INSIDE the full-screen renderer — what makes the wheel scroll the transcript. DEFAULT ON. Set `false` (or pass `--no-mouse`) to turn it off durably: the wheel stops scrolling (PgUp/PgDn + Ctrl+Home/Ctrl+End still page) and the emulator's native click-drag SELECTION works again without a modifier. The `--no-mouse` flag overrides this key. Ignored outside the alt screen — the inline renderer never enables mouse reporting. See [accessibility.md](../cli/accessibility.md) for the copy trade-off and the `/scrollback` + `/edit` hatches.
