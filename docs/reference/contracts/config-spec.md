...
alt_screen = true                  # ADR-0068 §e (2.6.F): opt into the full-screen alternate-screen renderer for the bare Home + `relavium chat`. false / the `--no-alt-screen` flag keep the byte-identical INLINE renderer (native scrollback + the emulator's own a11y — the screen-reader fallback). The `--no-alt-screen` flag overrides this key; a non-TTY / `--json` / CI path always renders inline. Absent ⇒ the phase default (opt-in until the viewport lands — ADR-0068 §b).
                                   # ⚠ PREVIEW at 2.6.F Step 4b-1: the transcript now renders through a tail-following, resize-tracked VIEWPORT (Step 4b-1), but you cannot SCROLL BACK yet — PgUp/PgDn/Ctrl+Home-End (Step 4b-2) and the `[`-dump / `v`-open-in-$EDITOR copy-and-search hatches (Step 5) are still pending, and the alt buffer has no native scrollback, so history that scrolls off the top is not yet reachable. Fully usable once scroll lands. Leave it off (the default) unless you want the full-screen preview.
