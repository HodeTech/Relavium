...
- `{{run.id}}` — the run's id (1.AF) — for an output node's `save_to` path template (e.g. `out/{{ run.id }}/image.png`). The interpolation engine (1.L2) recognizes the `run.id` namespace alongside `run.outputs`; it resolves only where the run id is in scope (today: `save_to`), so a `{{ run.id }}` elsewhere surfaces a typed `unresolved_reference` rather than an empty string.
...
> **`save_to` on an `output` node (1.AF, [ADR-0031](../../decisions/0031-llm-seam-shape-amendment-multimodal-io.md) A9 / [ADR-0044](../../decisions/0044-media-access-governance-read-media-save-to-cost.md)).** An optional **relative** path template the surface writes generated media bytes to (e.g. `save_to: 'out/{{ run.id }}/image.png'`). It interpolates **only `{{ run.id }}`** — a filesystem-path template deliberately does not draw arbitrary `inputs`/`ctx`/`run.outputs` data into the path (narrowing the injection surface; the host jail below is the binding control). **Authoring fail-fast:** an absolute path (`/…`), a Windows drive (`C:\` / `C:/`), a UNC path (`\\…`), or a `..` traversal segment is rejected at parse. The DAG edge and every run event carry the media **handle**, never bytes — `save_to` materializes bytes only at the **surface** boundary, through a host write port that additionally enforces `realpath`+`commonpath` fail-closed against a scope root, symlinks off (security-review.md §Media byte delivery). The engine resolves the single produced media handle to bytes (`MediaStore.get`) and hands `(relativePath, bytes)` to the host `MediaWritePort`; a `save_to` whose captured output has no handle — or more than one — fails the run (it is a real deliverable, not best-effort). Exported YAML therefore carries a handle or a relative `save_to` path, never bytes ([ADR-0009](../../decisions/0009-git-native-workflow-yaml.md)).
...
- id: security-scan-node
  ...
  count: 1                            # optional (1.AG Section C): images per call — generative model only
  duration_seconds: 8                # optional (1.AG Section C): audio/video length — generative model only
  timeout_ms: 60000
...
- id: merge
  ...
  merge_fn: |                        # required only when strategy = custom (a `js` expression)
    { ...branches[0], ...branches[1] }   # `branches` = SURVIVING branch outputs in branchNodeIds order
```

`merge_strategy` is the **authoritative** name for the aggregation behavior; the canvas (`AggregatorNode`) and engine (`fan_in_config`) carry the same value — see the [merge-strategy reconciliation table](../shared-core/node-types.md#merge-strategy-reconciliation). `best_of_n` (a secondary-LLM picker) is **reserved, not v1.0**, and is not a valid `merge_strategy` value in `'1.0'`. A custom `merge_fn` is a **bare `js` expression** (not a function) evaluated in the [expression sandbox](../shared-core/expression-sandbox-spec.md); it receives `branches` — the branch outputs in the engine's **stable `branchNodeIds` order** (never arrival order, so the merge is deterministic) — plus `run.outputs`. That order is the paired `parallel`'s `parallel_of` order **only when the merge joins exactly that parallel's branches**; otherwise it is the merge's own incoming branches in authored order, with any extra non-parallel branch appended — the canonical rule lives in [run-plan.md §fan-in branch order](../shared-core/run-plan.md).

> **Indices into `branches` are relative, not absolute.** A branch a `condition` routed away from is **omitted** from the array (the survivors keep their relative order), so `branches[1]` is not pinned to any particular authored node. Write a `merge_fn` that tolerates a shorter array — or reach for `object_merge`, which is keyed rather than positional — whenever a branch can be skipped.
...
workflow:
  ...
  budget:
    ...
    strict_cost_cap: false           # ADR-0071 §K7 + ADR-0089 §4: refuse a turn we cannot price (default false)
  timeout_ms: 300000                 # whole-run wall-clock cap
  max_parallel: 4                    # max concurrent in-flight LLM calls (omitted ⇒ 8; ceiling 64)
```

The cost cap is **pre-egress**: before each LLM call the engine checks `cumulative + worstCaseNextEstimate(maxTokens)` against `max_cost_microcents` and applies `on_exceed` — `fail` stops the run, `pause_for_approval` suspends it like a human gate (resumed via `resume_budget`), `warn` proceeds after a `budget:warning` event. An **unpriced model** (a custom `base_url`, or one the catalog does not carry) is a hole in the cap: the governor cannot estimate a turn's cost, so it **degrades to `allow` with a one-time notice** (ADR-0071 §K7) rather than hard-failing an otherwise-valid run. `strict_cost_cap: true` inverts that — an unpriced model is refused pre-egress, for a run that must not proceed on a model it cannot bound. Since [ADR-0089](../../decisions/0089-media-correctness-four-boundaries.md) §4 the same rule covers an unpriced **modality**: a model the catalog prices for tokens but for which it carries no image / audio / video rate is refused for a turn that requests that media output, on the identical rationale — the cap cannot bound a charge it cannot estimate. Without it the token path refused and the media path could not, so a paid generation passed a cap that read as enforced. The refusal names the modality and the `relavium models pricing … --image / --audio / --video` flag that adds the rate; with strict **off**, the call is allowed with the same one-time notice an unpriced model gets, and `cost:updated` carries `priced: false` so the missing charge is visible rather than reported as zero. This is a **BYOK-local** safety rail, distinct from Phase-2 managed-mode billing ([ADR-0014](../../decisions/0014-managed-metering-quota-and-billing.md)). The `budget:warning` / `budget:paused` / `run:timeout` events are defined in [sse-event-schema.md](sse-event-schema.md).
...
| `condition` | — | **REJECTED at parse in v1.0** — the field was accepted by the schema and read by nothing, so it silently did nothing; use a `condition` NODE ([ADR-0093](../../decisions/0093-an-expression-sees-only-what-it-is-ordered-after.md)). |
