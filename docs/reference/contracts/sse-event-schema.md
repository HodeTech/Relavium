...
| `node:completed` | A node finished successfully. | `nodeId`, `output`, `tokensUsed: {input, output, model?}` (`model` only for LLM nodes), `durationMs`, `selected?` (a `condition`'s chosen target ids — the authoritative branch record checkpoint/resume restores from, 1.R), `attemptNumber?` |
...
export interface NodeCompletedEvent extends BaseEvent {
  ...
  durationMs: number;
  selected?: string[];      // a `condition` node only: the immediate target ids it routed to (the live branches). The authoritative record checkpoint/resume restores `selectedTargets` from (1.R).
