...
| `run:paused` | The run is suspended on **≥1 gate AND/OR ≥1 async media job** — the multi-suspension aggregate (parallel branches may each reach a gate or a media job). `pendingGateCount`/`gateIds[]` are `0`/empty for a media-only park; `pendingMediaJobNodeIds` lists nodes parked on the engine-owned `pollMediaJob` loop (1.AG Section D, [ADR-0045](../../decisions/0045-async-media-job-loop-poll-checkpoint-resume-cancel.md) §2). A resume disambiguates by registry: a gate by `gateId` (a decision), a media job by `nodeId` (a re-attach). | `pendingGateCount`, `gateIds[]`, `pendingMediaJobNodeIds[]?` |
