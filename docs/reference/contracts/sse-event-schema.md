...
export interface CostAttemptSettledEvent extends BaseEvent {
  ...
  inputTokens: number;
  outputTokens: number;
  costMicrocents: number;   // THIS attempt's realized charge — the delta a reader SUMS. 0 is reachable (unpriced/free)
  cumulativeCostMicrocents: number; // the run-wide total AFTER it — a display snapshot, NOT the restore source
