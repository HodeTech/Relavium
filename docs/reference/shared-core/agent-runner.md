...
The runner emits **no** `budget:*` / `run_timeout` (run-level, not in the in-node set). A pure **always-pass pre-egress hook** sits immediately before each seam call — the [ADR-0028](../../decisions/0028-workflow-resource-governance.md) insertion point **1.AC** fills.
