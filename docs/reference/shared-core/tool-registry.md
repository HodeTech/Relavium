...
5b. **Split off a media attachment**, if the dispatch returned one (`CR-50`,
   [ADR-0089](../../decisions/0089-media-correctness-four-boundaries.md) §1). A tool that answers with media
   returns a private-symbol envelope pairing an ordinary result with handle-only `DurableMediaPart`s; the
   split happens HERE, at the dispatch boundary, so every step below sees the plain result and never the
   envelope. The parts leave on `ToolDispatchOutcome.mediaAttachments` (branded untrusted) and the turn loop
   delivers them on one synthesized `user` message after **all** of the response's tool results. Only
   `read_media` produces one today. A **journaled** (tiered) dispatch returning an attachment is REFUSED:
   the journal round-trips JSON, so a replay would restore the result and deliver no media.
...
| node `tools:` narrowing | enforced at **plan build** when the host marks the grant complete (`BuildRunPlanOptions.toolGrantsFinal`), with `resolveGrant` as the dispatch floor for a directly-constructed executor ([ADR-0094](../../decisions/0094-a-tool-grant-is-checked-when-the-plan-is-built.md)); narrow-only, 0029(b)/[ADR-0023](../../decisions/0023-strict-authored-yaml-validation.md) | — |
...
same compile-time technique as the 0029(c) secret taint). **Both halves of a dispatch outcome are branded**:
`toolResult`, and — since `CR-50` — `mediaAttachments`, the handle-only media a tool asks the engine to
deliver on a synthesized `user` message
([ADR-0089](../../decisions/0089-media-correctness-four-boundaries.md) §1). The second needs the brand more
than the first: the model chose that handle, and the message it rides is a `user` position, so a directive
painted into an image arrives as user speech. It is unwrapped once, at the single placement site, and the
message is fenced with an opening and a closing line telling the model to treat the attachment as reported
content — the same discipline the compaction summary uses. The 1.O message-assembly layer can then place
