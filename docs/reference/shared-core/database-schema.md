...
Models offered by each provider, including pricing used for local cost tracking. The `*_per_mtok_microcents` columns are price **per million tokens, in integer micro-cents** (one micro-cent = 1e-8 USD = cents x 1,000,000; see the [money/cost convention](#sqlite-type-conventions)). The three `media_*_cost_microcents` columns are the projection of `ModelPricing.mediaOutputRates` (1.AF/D17, [ADR-0044](../../decisions/0044-media-access-governance-read-media-save-to-cost.md) §3) — integer micro-cents **per billed media-output unit** (per image, per audio-second, per video-second); **NULL** when the model has no metered media rate — and NULL means **unpriced**, not free. [ADR-0089](../../decisions/0089-media-correctness-four-boundaries.md) §4 replaced the old degrade-to-zero behaviour this sentence used to describe: a missing rate is now reported as an unpriced modality on both cost paths, counted in `unpriced_calls`, and **refused** under `strict_cost_cap` with a runnable remedy. A `0` written here is a *stated* free rate and is honoured as one; the two are no longer the same value. `document`/PDF is excluded (it bills as tokens). No shipped model carries a media rate yet, so these are NULL across the seeded catalog.
...
| `token_rates_stated` | INTEGER (bool) | NOT NULL DEFAULT 0 — did the USER state the token rates (migration 0016, [ADR-0089](../../decisions/0089-media-correctness-four-boundaries.md) §4)? The sibling of `cached_input_stated`, for the same reason: the two rate columns are `NOT NULL DEFAULT 0`, so "never said" and "stated free" are the same bytes. `0` ⇒ a reader inherits the catalog's rates; `1` ⇒ the numbers are the user's own, zero included. **Derived by the store** from the presence of both rates, so no writer can forget it. It became load-bearing when a media-only `models pricing` invocation became legal — without it, adding an image rate to a catalog-priced model wrote `0`/`0` into a row that outranks the catalog, billing every token on that model at nothing |
