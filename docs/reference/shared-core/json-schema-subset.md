...
| malformed `type` / `required` / `properties` / `enum` / `const` / `items` | **fails closed**, except a degenerate `type`: `lenient` accepts `type: []` as **unconstrained**, and `type: ['string','string']` as plain `string` (the duplicate collapses into the same union arm) | **fails closed** |
