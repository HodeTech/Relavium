...
| `first` | `first` | `first` | Take the first surviving branch **by declaration order**. The join still waits for EVERY branch to settle, so a slow or failing loser adds latency and cost ([ADR-0091](../../decisions/0091-first-means-first-declared-not-first-to-finish.md)); true early-cancel is a recorded future capability, not current behaviour. |
