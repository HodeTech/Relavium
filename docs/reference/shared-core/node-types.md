...
| `first` | `first` | `first` | Take the first surviving branch **by `branchNodeIds` declaration order** — the paired `parallel`'s `parallel_of` order when the merge joins exactly that parallel's branches, else the merge's own incoming branches in authored order (see [run-plan.md §fan-in branch order](run-plan.md)). The join still waits for EVERY branch to settle, so a slow loser extends the run to the **slowest** branch, and a **failing loser fails the whole run** even when the winner has already produced its output. `first` is a combine strategy, **not failover** ([ADR-0091](../../decisions/0091-first-means-first-declared-not-first-to-finish.md)); true early-cancel is a recorded future capability, not current behaviour. |
