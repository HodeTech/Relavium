...
- Older binaries cannot necessarily replay a log containing the new event discriminant. Current readers already require forward-compatible handling of unknown event types; deploys must avoid an unsafe downgrade while a run/session with new commitments is active.
