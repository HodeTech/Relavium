...
- The per-node agent turn ([`agent-turn.ts`](../../packages/core/src/engine/agent-turn.ts)) consumes **`chain.stream()` only**; it folds `text` / `tool_call` / `reasoning` chunks into the result — there is no media path and no `generate()` path on the `FallbackChain` today.
...
2. **The `FallbackChain` gains a `generate()` method.** It was stream-only; `generate()` reuses the same provider skip/select, cross-provider failover, per-attempt cost (`CostTracker`/`PreEgressHook`), and `LlmError` classification as `stream()`, in one round-trip. *(Considered: synthesise a non-streaming result by draining `stream()` — rejected: the media never rides a `StreamChunk`, so draining yields no media; `generate()` is the seam method that returns `LlmResult.content`.)*
...
The `StopReason` rule is unchanged (ADR-0031 §5.3): a media-only turn reports `'stop'`; the signal is a `media` part in `content`, never a new stop reason — consumers inspect `content`, and the engine compares produced modalities against the node's `output_modalities` for a missing-modality route/retry.
...
- A turn that both calls tools **and** emits inline media in one shot is the awkward case; 1.AG handles the terminal media-output turn (no further tool round), consistent with how media-gen is authored.
