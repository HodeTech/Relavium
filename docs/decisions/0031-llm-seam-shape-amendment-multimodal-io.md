...
> **Amended 2026-09-02 by [ADR-0089](0089-media-correctness-four-boundaries.md).** A refinement, not a reversal, on the **`read_media`** path only. The model-facing tool narrows to a **whole handle** (its `start`/`end` parameters leave the tool, because a byte range has no representation in a handle-only result), and the bytes reach the model as an ordinary **top-level `media` `ContentPart` on a marked, engine-synthesized `user` message** — the media-input path this ADR already established, which every adapter lowers and the chain already re-materializes before egress. Decision **#7's `tool_result.media` arm is unchanged and untouched**: it stays reserved for the **provider-executed** case it was built for (an image-gen result the provider itself returns inside a tool result), which `read_media` — engine-executed — is not. The `Range` primitive keeps its real caller, the desktop display command. The seam shape here is unchanged.

> **Amended 2026-07-05 by [ADR-0064](0064-live-model-catalog.md).** A refinement, not a reversal: the seam gains one more **optional capability method** — `listModels?` (live model discovery, returning a Relavium-typed `ModelListing[]` mapped inside each adapter — no vendor type crosses) — in the same additive spirit as the reserved media methods here. The multimodal seam shape is unchanged.
