...
**That last point carries a doc obligation, not just a caution.** The comment above the telescoping delta in `run-history-store.ts` currently justifies it by out-of-order COMMIT — the thing §1 removes — so the moment the tail lands, the money write's own stated reason is contradicted by the code beside it, and the next reader deletes the telescoping. CR-10's implementation rewrites that comment to the real reason (a stamp-time capture, unaffected by ordering), alongside §6's two re-derivations.
