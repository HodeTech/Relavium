...
**The managed gateway meters usage in real time in Redis with a reserve→settle protocol keyed on a unique `request_id`, enforces quotas and per-user/day budgets in line, captures streaming usage robustly, reconciles nightly against provider invoices, and bills through a merchant-of-record (Paddle / Lemon Squeezy) as the primary rail — with a direct Stripe integration as the mutually-exclusive alternative. Every `usage_event` stores both `provider_cost` and `billed_cost`, so margin is observable per request.**

> Amended 2026-06-04: MoR primary. The merchant-of-record is the primary/default billing rail; Stripe is the mutually-exclusive alternative, never layered. See the "Billing topology (clarification)" note below, which this headline is now consistent with.
...
- **Billing rail (MoR primary).** Subscriptions, included usage, and metered overage / prepaid credit top-ups run through a **merchant-of-record (Paddle / Lemon Squeezy)** as the **primary/default** rail — it is the legal seller-of-record and fronts VAT/sales-tax/chargebacks ([ADR-0015](0015-managed-mode-data-handling-and-compliance.md)). A direct **Stripe** integration is the **mutually-exclusive alternative**, used only if not going through an MoR; the two are never layered. The plan shape from [ADR-0012](0012-managed-inference-dual-mode.md) is prepaid credits + hard included cap + overage at ~cost×1.3. See the "Billing topology (clarification)" note below.
...
3. **Real-time Redis metering + reserve/settle idempotent on `request_id` + nightly reconciliation, billed through a merchant-of-record (Stripe as the mutually-exclusive alternative)** — *chosen.* The only shape that enforces caps before the next request, keeps the prepaid float non-negative under interruption and retries, and ties out to provider invoices.
...
### Billing topology (clarification)

An earlier draft of the **Decision** above billed managed usage "through Stripe"; the Decision now carries a dated `> Amended 2026-06-04` note (per the amend-vs-supersede rule in [documentation-style.md](../standards/documentation-style.md) §7) making the merchant-of-record the primary rail, and this clarification spells the topology out in full. The **merchant-of-record (Paddle / Lemon Squeezy) is the PRIMARY seller-of-record and billing rail** — it is the legal seller, and it absorbs VAT/sales-tax, chargebacks, and disputes ([ADR-0015](0015-managed-mode-data-handling-and-compliance.md), [tax-and-billing.md](../compliance/tax-and-billing.md)). A direct **Stripe** integration is the mutually-exclusive **ALTERNATIVE** rail, used **only if not going through an MoR**; Stripe and an MoR are never layered (one in front of the other). The internal `usage_events` ledger is **not a billing rail**: it **meters** managed consumption (reserve→settle, idempotent on `request_id`) and **feeds invoicing through whichever single rail is configured**. Pinned in [tech-stack.md](../tech-stack.md) (MoR-primary, Stripe-alternative).
