...
2. **Pick the next free number — dynamically.** Numbers are zero-padded 4 digits and never reused or renumbered. Compute the next one from the live tree; never assume a hardcoded head, and never copy a number from this skill's examples.
   ```bash
   R=$(git rev-parse --show-toplevel)
   NEXT=$(printf '%04d' $(( 10#$(ls "$R"/docs/decisions/ | grep -E '^[0-9]{4}-' | sort | tail -1 | cut -c1-4) + 1 )))
   echo "$NEXT"   # the next free ADR number — trust this, not any number written below
   ```
   Use `$NEXT` everywhere a number appears below. The `NNNN` in the examples is a placeholder.
3. **Copy the template** into the new file and rename with a kebab slug:
   ```bash
   cp "$R"/docs/standards/adr-template.md \
      "$R"/docs/decisions/"$NEXT"-your-slug.md
   ```
4. **Fill the H1 and metadata.** `# ADR-NNNN: Title` (with your computed `$NEXT`), then bold `Status` / `Date` (ISO 8601, e.g. `2026-06-03`) / `Related` lines linking sibling ADRs and the framing `product-constraints.md` / `architectural-principles.md` clauses by relative path. Set Status to **Accepted** when settled; do not do a ceremonial Proposed→Accepted round-trip.
...
8. **Update the index.** Add a row to the table in `docs/decisions/README.md` (`| NNNN | [Title](NNNN-your-slug.md) | Accepted | 2026-06-03 |`, with your computed `$NEXT`) in numerical order.
...
10. **Commit** with ../commit-and-pr/SKILL.md: `docs(decisions): add ADR-NNNN <topic>` and a `Refs: ADR-NNNN` trailer (use your computed `$NEXT`).
...
- Worked supersede example (0004→0011): the superseded ../../../docs/decisions/0004-vercel-ai-sdk-multi-llm.md (its `Superseded by` header) and the superseding ../../../docs/decisions/0011-internal-llm-abstraction.md
