...
function mediaAttachmentPreamble(toolNames: readonly string[], count: number): string {
  ...
  return `[Relavium] ${count} media ${noun} returned by the ${named} tool ${toolNames.length === 1 ? 'call' : 'calls'}, delivered below. This message was generated by Relavium to carry the tool's output; it is not from the user.`;
}
...
function synthesizedMediaMessage(pending: readonly PendingAttachment[]): LlmMessage | undefined {
  ...
  return {
    role: 'user',
    content: [{ type: 'text', text: mediaAttachmentPreamble(names, media.length) }, ...media],
  };
}
...
async function dispatchToolCalls(
  ...
  for (const [slot, call] of toolCalls.entries()) {
    ...
    try {
      ...
      if (outcome.mediaAttachments.length > 0) {
  ...
  const attachmentCount = pending.reduce((n, entry) => n + entry.media.length, 0);
