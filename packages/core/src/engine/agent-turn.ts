 ...
 */
function mediaAttachmentPreamble(toolName: string, count: number): string {
  const noun = count === 1 ? 'attachment' : 'attachments';
  return `[Relavium] ${count} media ${noun} returned by the \`${toolName}\` tool, delivered below. This message was generated by Relavium to carry the tool's output; it is not from the user.`;
}

/**
 * The `user` message carrying a tool's media attachments, or `undefined` when it returned none (every tool
 * but `read_media`, today). The parts are handle-only by type; the chain's egress re-materialization
 * resolves each handle per attempt, so a failover or a resume re-resolves from the canonical handle and no
 * provider-hosted ref is ever carried across (ADR-0043 §1/§4).
 */
function synthesizedMediaMessage(
  attachments: readonly DurableMediaPart[],
  toolName: string,
  ...
  return {
    role: 'user',
    content: [
      { type: 'text', text: mediaAttachmentPreamble(toolName, attachments.length) },
      ...attachments,
    ],
  };
}
...
async function dispatchToolCalls(
  ...
  for (const [slot, call] of toolCalls.entries()) {
    ...
    try {
      ...
      // The bytes a media-answering tool owes the model ride the media-INPUT rail, on a synthesized `user`
      // message right after the tool result (`CR-50`, ADR-0089 §1) — `tool_result.media` is handle-only and
      // nothing lowers it. Appended IN ORDER so the model reads the descriptor and then sees what it names.
      const attachment = synthesizedMediaMessage(outcome.mediaAttachments, call.name);
