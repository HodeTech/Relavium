...
import { catalogMeta, llmProviders, modelCatalog, modelMetadata, type NewModelMetadataRow } from './schema.js';
...
describe('model_metadata / catalog_meta schema constraints (ADR-0072 P1)', () => {
  ...
  it('admits a priced refreshed long-tail row', () => {
    client.db.insert(modelMetadata).values(row({ modelId: 'gpt-6-mini', origin: 'refreshed' })).run();
    const got = client.db.select().from(modelMetadata).where(eq(modelMetadata.modelId, 'gpt-6-mini')).all();
  ...
  it('REJECTS a refreshed row with a zero OR negative base price (the gpt-5.5 $0.00 regression class, at rest)', () => {
    expect(() =>
      client.db.insert(modelMetadata).values(row({ modelId: 'free-longtail', inputCostPerMtokMicrocents: 0 })).run(),
    ).toThrow(/CHECK|constraint/i);
    expect(() =>
      client.db.insert(modelMetadata).values(row({ modelId: 'free-longtail-2', outputCostPerMtokMicrocents: 0 })).run(),
    ).toThrow(/CHECK|constraint/i);
    // A NEGATIVE rate bounds the CHECK's direction, not just its zero edge — it must fail exactly the same way.
    expect(() =>
      client.db.insert(modelMetadata).values(row({ modelId: 'neg-longtail', inputCostPerMtokMicrocents: -1 })).run(),
    ).toThrow(/CHECK|constraint/i);
  });
  ...
  it('ADMITS a shipped row with a zero base price (a free shipped model is legitimate and pinned)', () => {
    client.db
      ...
      .values(row({ modelId: 'free-shipped', origin: 'shipped', inputCostPerMtokMicrocents: 0, outputCostPerMtokMicrocents: 0 }))
      .run();
    const got = client.db.select().from(modelMetadata).where(eq(modelMetadata.modelId, 'free-shipped')).all();
  ...
  it('REJECTS an unknown origin value', () => {
    ...
    const causeMsg = caught instanceof Error && caught.cause instanceof Error ? caught.cause.message : '';
  ...
  it('round-trips NULL cache columns as null, never 0 (ADR-0071 §10)', () => {
    client.db
      .insert(modelMetadata)
      .values(row({ modelId: 'no-cache-rate', cachedInputCostPerMtokMicrocents: null, cacheWriteCostPerMtokMicrocents: null }))
      .run();
    const got = client.db.select().from(modelMetadata).where(eq(modelMetadata.modelId, 'no-cache-rate')).all();
  ...
  it('defaults model_catalog.visible to true on an existing/omitting-insert row (the ALTER ADD DEFAULT 1)', () => {
    ...
    client.db
      .insert(llmProviders)
      .values({ id: 'p1', name: 'openai', displayName: 'OpenAI', baseUrl: 'https://api.openai.com/v1', createdAt: TS_MS, updatedAt: TS_MS })
      .run();
    client.db
      .insert(modelCatalog)
      .values({
  ...
  it('enforces the catalog_meta singleton — BOTH halves: CHECK (id != 1) and PK (duplicate id = 1)', () => {
    ...
    expect(() => client.db.insert(catalogMeta).values({ id: 2, updatedAt: TS_MS }).run()).toThrow(/CHECK|constraint/i);
    // The PRIMARY KEY half: a SECOND id = 1 is a PK collision — together with the CHECK this pins "exactly one row".
    expect(() => client.db.insert(catalogMeta).values({ id: 1, updatedAt: TS_MS }).run()).toThrow(/constraint|UNIQUE|PRIMARY/i);
    expect(client.db.select().from(catalogMeta).all()).toHaveLength(1);
  });
});
