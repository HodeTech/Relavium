...
describe('createModelRefreshService', () => {
  ...
  it('never leaks a provider key into the report (or an error message)', async () => {
    const resolver = stubResolver({
      adapters: {
        anthropic: stubProvider('anthropic', () =>
          // Even a failing adapter whose error TEXT mentions the key: the service surfaces `error.message`, which
          // the real seam redacts. Here we assert the service never fabricates a key into the report itself.
          Promise.reject(new Error('list failed for provider')),
        ),
